...
class GoLogin(object):
    def __init__(self, options):
        ...
        self.executablePath = options.get('executablePath', os.path.join(
            home, '.gologin/browser/orbita-browser/chrome'))
    ...
    def convertPreferences(self, preferences):
        ...
        if preferences.get('navigator', {}).get('language'):
            preferences['language'] = preferences.get(
    ...
    def waitDebuggingUrl(self, delay_s, try_count=3):
        ...
        wsUrl = wsUrl.replace(
            'ws://', 'wss://').replace('127.0.0.1', self.profile_id + '.orbita.gologin.com')
        return {'status': 'success', 'wsUrl': wsUrl}

    def startRemote(self, delay_s=3):
        profileResponse = requests.post(
            API_URL + '/browser/' + self.profile_id + '/web', headers=self.headers()).content.decode('utf-8')
    ...
    def stopRemote(self):
        requests.delete(API_URL + '/browser/' +
                        self.profile_id + '/web', headers=self.headers())
